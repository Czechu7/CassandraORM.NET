...
public class CassandraDbSet<T> : IQueryable<T>, IAsyncEnumerable<T> where T : class
{
    ...
    {
        return _queryProvider.Execute<IEnumerable<T>>(_expression).GetEnumerator();
    }
    ...
    public void Remove(T entity)
    {
        ...
        {
            // If the entity was just added, remove it from tracking
            _context.TrackEntity(entity, EntityState.Detached);
        }
        else
        {
            _context.TrackEntity(entity, EntityState.Deleted);
        }
    }
    ...
    {
        return FindAsync(keyValues, CancellationToken.None);
    }
    ...
    public async Task<IEnumerable<T>> FromCqlAsync(string cql, params object[] parameters)
    {
        ...
        foreach (var row in result)
        {
            entities.Add(MapRowToEntity(row, metadata));
        }
