...
/// </summary>
public class CassandraRetryPolicy
{
    private readonly ILogger? _logger;

    public CassandraRetryPolicy(ILogger? logger = null)
    {
        _logger = logger;
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        while (true)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    _logger?.LogError(ex, "Operation failed after {MaxAttempts} attempts", maxRetries + 1);
                    throw new InvalidOperationException(
                        $"Operation failed after {maxRetries + 1} attempts. Last error: {ex.Message}", ex);
                }
                
                // Non-retryable exception
                _logger?.LogError(ex, "Operation failed with non-retryable exception: {Error}", ex.Message);
                throw;
            }
        }
    }
...
public static class RetryExtensions
{
    ...
    {
        return new CassandraDbContextWithRetry(context, new CassandraRetryPolicy(logger));
    }
}
...
/// </summary>
public class CassandraDbContextWithRetry : IDisposable
{
    private readonly CassandraDbContext _context;
    private readonly CassandraRetryPolicy _retryPolicy;

    public CassandraDbContextWithRetry(CassandraDbContext context, CassandraRetryPolicy retryPolicy)
    {
